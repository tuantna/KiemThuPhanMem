...
namespace DoAnTotNghiep.BusinessLayer
{
    public class BLPKiemKe
    {
        ...
        Table<PHIEUKIEMKE> query;
        public BLPKiemKe()
        {
            query = PhamMem.db.PHIEUKIEMKEs;
        }
        #endregion

        #region Xử lý đọc dữ liệu
        public IList<PHIEUKIEMKE> DocPKKeTheoTBID(int pTBID)
        {
            IList<PHIEUKIEMKE> dh = query.Where(x => x.ThietBiID == pTBID).ToList();
            return dh;
        }
        ...
        public IList<PHIEUKIEMKE> Them(PHIEUKIEMKE pCTPNhap)
        {
