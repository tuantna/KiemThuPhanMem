...
namespace DoAnTotNghiep.BusinessLayer
{
    public class BLCTPNhap
    {
        ...
        Table<CHITIETPNHAP> query;
        public BLCTPNhap()
        {
            query = PhamMem.db.CHITIETPNHAPs;
        }
        #endregion
        ...
        public IList<CHITIETPNHAP> DocCTPNTheoID(int pTBID)
        {
            IList<CHITIETPNHAP> dh = query.Where(x => x.PhieuNhapID == pTBID).ToList();
            return dh;
        }
        public IList<CHITIETPNHAP> DocCTPNTheoTBID(int pTBID)
        {
            IList<CHITIETPNHAP> dh = query.Where(x => x.ThietBiID == pTBID).ToList();
            return dh;
        }
        ...
        public IList<CHITIETPNHAP> Them(CHITIETPNHAP pCTPNhap, int pSL)
        {
            if (CTPNhapHopLe(pCTPNhap, pSL))
            {
                query.InsertOnSubmit(pCTPNhap);
